...
export function LessonHomeworkSection({ lessonId }: { lessonId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/homework?lessonId=${encodeURIComponent(lessonId)}`)
      .then((r) => r.json())
  ...
  async function handleSubmitLink(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/homework/submit", {
        ...
        body: JSON.stringify({ lessonId, type: "link", linkUrl: url }),
      });
  ...
  async function handleSubmitFile(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/homework/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ lessonId, type: mode, fileUrl: fileUrl.trim(), fileName: fileName.trim() || undefined }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setError(data.error ?? "فشل التسليم");
      ...
      router.refresh();
      setSubmissions((prev) => [
        { id: "", submission_type: mode, link_url: null, file_url: fileUrl, file_name: fileName || null, created_at: new Date().toISOString() },
        ...prev,
      ]);
    } catch {
      setError("حدث خطأ");
  ...
  return (
    <div className="mt-6 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
      <h2 className="text-lg font-semibold text-[var(--color-foreground)]">
        إرسال الواجب
      </h2>
      <p className="mt-1 text-sm text-[var(--color-muted)]">
        يمكنك إرسال رابط أو ملف PDF أو صورة كواجب لهذه الحصة.
      </p>
      {error && (
